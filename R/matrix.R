...
#' @description Return alignment list object from reading in sequences from a
#' single FASTA file or multiple FASTA files.
#' @param flpths File path(s) to .fasta file(s)
#' @return alignment list
#' @export
alignment_read <- function(flpths) {
  read <- function(flpth) {
    ...
    for (i in seq_along(all_data)) {
      ...
      if (grepl(pattern = '^>', x = bit)) {
        ...
        alignment[[nm]] <- c(alignment[[nm]], bit)
      }
    }
    lngths <- vapply(X = alignment, FUN = length, FUN.VALUE = integer(1))
    if (all(lngths != lngths[[1]])) {
      msg <- paste0(char(flpth), 'is not an sequences have different lengths.')
      stop(msg)
    }
    res <- matrix(unlist(alignment), ncol = length(alignment[[1]]),
                  nrow = length(alignment),
                  byrow = TRUE)
    rownames(res) <- names(alignment)
    class(res) <- 'alignment'
    res
  }
  res <- lapply(X = flpths, FUN = read)
